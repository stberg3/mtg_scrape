...
class DownloadThread(threading.Thread):
    def __init__(self, url, store_directory):
        ...
        def run(self):
            fname = str(self.store_directory+"/"+
            (sha1(self.url.encode("utf8")).hexdigest()) + ".jpg")
...
if not os.dir.isdir(store_directory):
...
for i in range(2):
    for i in range(4):
        ...
        time.sleep(1)
    try:
        # Click the button that makes more results appear
